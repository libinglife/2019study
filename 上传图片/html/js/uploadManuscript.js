...
function sub(){
    ...
    var url = "https://game-renrenaikan.renren.com/VideoBack/UpLoadVideo"; // 接收上传文件的后台地址
...
function uploadComplete(evt) {
    ...
    if(evt.code==0){
        ...
        var str = baseUrl+evt.url.substr(7).replace(/\\/g,'/');
        videoUrl = evt.url
...
function submitFile() {
    ...
    $.ajax({
        ...
        // url: "http://120.133.2.217:8080/VideoBack/VideoCreate" ,//url
        url: "/VideoBack/VideoCreate" ,//url
        data:{
            title:$('#title').val(),
            address:videoUrl,
            envelope:imgUrl,
            maintag:$('#showPath').val(),
            tag:$('#tagsinputval').val()
        },
